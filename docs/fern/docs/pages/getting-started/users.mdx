...
<Note>
  ...
  - **Server Components**: If a component is protected, it is guaranteed that its bundled HTML will not be sent to browser. However, this is not necessarily true for its children and the rest of the page, as Next.js may split components on the same page and send them to the client separately for performance.
  
    For example, if your page is `<Parent><Child /></Parent>`, where `Parent` is protected and `Child` is not, Next.js may still send `<Child />` to the browser. (Normal browsers will never display it, but attackers may be able to retrieve it.)
